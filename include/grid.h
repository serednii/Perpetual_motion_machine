void grid() {
  const int column = cell, row = cell;
  const int gridColor = 0x000000;
  ...
  rect(buffer0, column, row, column + width, row + height, gridColor);
  ...
  rect(buffer, xx2, yy2, xx2 + width1, yy2 + height1, gridColor);
